...
from olwriter import writeOL, exportLayers
...
class MainDialog(QDialog, Ui_MainDialog):
    ...
    def updateDeployGroups(self):
        postGisMethods = [utils.METHOD_WMS_POSTGIS]
        geoServerMethods = [utils.METHOD_WMS_POSTGIS, utils.METHOD_WMS]
        usesGeoServer = False
        usesPostGis = False
    ...
    def __init__(self, layer, tree):
        ...
        if layer.type() == layer.VectorLayer:
            if layer.providerType().lower() != "wfs":
                ...
                options = ["Use file directly", "GeoServer->WMS", "PostGIS->GeoServer->WMS"]
            ...
            self.popupLabel.setDisabled(True)
            self.popupItem.setDisabled(True)
